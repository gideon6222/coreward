...
**`audio.ts` is fully strict too, with no non-null assertions.** Chrome blocks an
AudioContext created outside a user gesture, so the whole graph is built lazily
by `audioInit()` on the first touch. It is built in one go and never partially,
so it is modelled as a single `Graph | null`: one check narrows all eleven nodes
at once, and the guards the code already had are the guards the type system
reads. `A.drill` and `A.timer` sit outside the graph because they genuinely
change while the game runs.

The smoke test covers it by counting `createBufferSource` calls through a
wrapped `AudioContext` — counting contexts alone would still pass if the graph
were built and then never published, since every sound would silently no-op.
